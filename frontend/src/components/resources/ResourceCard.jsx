...
const ResourceCard = ({ resource }) => {
  ...
  const getCategoryColor = (category) => {
    const colors = {
      'spatial-audio': 'bg-purple-100 text-purple-800',
  ...
  return (
    <div className="bg-white rounded-lg shadow-md hover:shadow-xl transition-shadow duration-300 overflow-hidden">
      ...
      <div className="p-5">
        ...
        {resource.author && (
          <div className="flex items-center gap-2 pt-4 border-t border-gray-100">
            ...
            <span className="text-sm text-gray-600">
              {resource.author.name} • {resource.author.university}
            </span>
          </div>
        )}
        ...
        <button 
          onClick={() => {
            // Handle specific known resources first
...
*Generated by Spatial AI Platform - Educational Resource Library*`;
            ...
            if (resource.type === 'video') {
              if (resource.url) {
                ...
                alert(`🎥 Opening video: "${resource.title}"\n\nThis would normally open the video player or redirect to the video content.`);
              }
